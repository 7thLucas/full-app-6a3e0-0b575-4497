...
export class AuthService {
  ...
  static async getUsers(page = 1, limit = 20): Promise<{ users: PublicUser[]; total: number; page: number; limit: number }> {
    const skip = (page - 1) * limit;
    const [docs, total] = await Promise.all([
      UserModel.find().sort({ createdAt: -1 }).skip(skip).limit(limit),
      UserModel.countDocuments(),
    ]);
    return { users: docs.map(toPublicUser), total, page, limit };
  }
